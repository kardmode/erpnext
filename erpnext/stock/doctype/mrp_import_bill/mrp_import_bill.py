...
def create_condensed_table(items):	
	...
	joiningtext = """<div>""" + str(posting_date) + """ - """ + str(posting_time) + """</div><br>"""
	...
	for item in new_list:
		...
		if item_code in item_dict:
			...
			if item_alt:
				...
				if item_alt in item_dict[item_code]["item_alts"]:
...
def get_bills_and_stock(item_code=None,company = None,posting_date=None,posting_time=None):
	...
	for item in new_list:
		...
		if import_bill in item_dict:
			if item_alt:

				if item_alt in item_dict[import_bill]["item_alts"]:

...
@frappe.whitelist()
def mrp_get_items(warehouse, posting_date, posting_time, company, item_group=None, ignore_empty_stock=False, ignore_positive_stock=False, ignore_negative_stock=False):
	ignore_empty_stock = cint(ignore_empty_stock)
	ignore_positive_stock = cint(ignore_positive_stock)
	ignore_negative_stock = cint(ignore_negative_stock)
	items = []
	items = get_items_for_stock_reco(warehouse, company)
	item_code=None
	res = []
	...
	# Get allowed item groups (parent + children)
	allowed_groups = []
	if item_group:

		allowed_groups = get_descendants_of("Item Group", item_group, ignore_permissions=True)
		allowed_groups.append(item_group)
	...
	for d in items:
		...
		if item_group:
			item_group_of_item = frappe.db.get_value("Item", d.item_code, "item_group")
			if item_group_of_item not in allowed_groups:
				continue
		...
		if d.item_code in itemwise_batch_data:	
			...
			for row in itemwise_batch_data.get(d.item_code):
				...
				# Skip by stock filter
				if ignore_empty_stock and qty == 0:
					continue
				if ignore_positive_stock and qty > 0:
					continue
				if ignore_negative_stock and qty < 0:
					continue
